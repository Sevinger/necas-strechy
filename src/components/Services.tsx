          ...
          {services.map((s, i) => (
            ...
            >
              <div className="relative aspect-[4/5] overflow-hidden bg-muted">
                ...
                />
                <div className="absolute top-4 left-4 bg-cream/90 backdrop-blur-sm px-3 py-1.5 rounded-full text-[10px] uppercase tracking-[0.2em] text-ink">
                  {s.badge}
                </div>
              </div>
              <div className="p-6">
                <h3 className="font-display text-2xl text-ink leading-tight mb-2">{s.title}</h3>
                <p className="text-xs uppercase tracking-[0.18em] text-copper mb-4">{s.subtitle}</p>
                <div className="flex flex-wrap gap-x-4 gap-y-1.5 pt-4 border-t border-border text-xs text-muted-foreground">
                  {s.meta.map((m, idx) => (
